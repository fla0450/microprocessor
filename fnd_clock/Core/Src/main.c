...
uint32_t cur_tick = 0, pre_tick = 0;
uint32_t h = 23;
uint8_t m = 59;
/* USER CODE END PV */
...
int main(void)
{
  ...
  while (1)
  {
	  cur_tick = HAL_GetTick();
	  	  ...
	  	  c = h/10;
	  	  d = h%10;
	  	  display_fnd(number[c], 3, 5);
	  	  display_fnd(number[d]|0x80, 2, 5);
	  	  display_fnd(number[a], 1, 5);
	  	  display_fnd(number[b], 0, 5);
    /* USER CODE END WHILE */
